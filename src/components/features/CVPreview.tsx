...
export function CVPreview({ 
  ...
  // Default to single column if no layout is provided
  ...
  const renderSection = (id: CVSectionId) => {
    switch (id) {
          ...
          <section key="skills">
            ...
            <div className="flex flex-wrap gap-1.5">
              {data.relevantSkills.map((skill, index) => (
                <span key={index} className="px-2.5 py-0.5 bg-white text-black text-xs font-semibold border border-black rounded-sm">
                  {skill}
                </span>
              ))}
            </div>
      ...
      case "projects":
        if (!data.selectedProjects || data.selectedProjects.length === 0) return null;
        return (
                  ...
                  {proj.techStack && proj.techStack.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {proj.techStack.map((tech, idx) => (
                        <span key={idx} className="px-2.5 py-0.5 bg-white text-black text-xs font-semibold border border-black rounded-sm">
                          {tech}
                        </span>
                      ))}
                    </div>
  ...
  return (
    <div className="flex flex-col min-h-full font-sans text-slate-800 bg-white">
      ...
      {/* Main Content */}
      <div className={`flex-1 ${currentLayout.mode === 'two-column' ? (
        currentLayout.ratio === 'equal' ? 'grid grid-cols-2 gap-8 print:grid-cols-2' :
        currentLayout.ratio === 'right-narrow' ? 'grid grid-cols-[2.5fr_1fr] gap-8 print:grid-cols-[2.5fr_1fr]' :
        ...
        {currentLayout.mode === 'single' ? (
          ...
          <>
            ...
            <div className="space-y-8 print:space-y-6">
              {currentLayout.rightColumn.map(renderSection)}
            </div>
          </>
        )}
      </div>
