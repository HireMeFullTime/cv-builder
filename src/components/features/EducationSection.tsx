		...
		<div className='space-y-6'>
			...
			<div className='space-y-4'>
				{initialEducations.map(edu => {
					if (editingId === edu.id) {
						return <EducationForm key={edu.id} initialData={edu} onClose={() => setEditingId(null)} />;
					}
					...
					return (
						<Card key={edu.id}>
									...
									<div>
										<CardTitle className='text-lg flex items-center gap-2'>
											...
											{edu.isCurrent && (
												<Badge
													variant='outline'
													className='ml-2 text-[10px] font-normal uppercase tracking-wider shrink-0'
												>
													Current
												</Badge>
											)}
							...
							<CardContent className='pb-4'>
								...
								{edu.description && (
									<p className='text-sm text-muted-foreground mb-3 whitespace-pre-wrap'>{edu.description}</p>
								)}
